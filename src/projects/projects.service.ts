...
export class ProjectsService {
  ...
  async create(userId: string, name: string): Promise<SanitizedProject> {
    ...
    return await this.prisma.$transaction(async (tx) => {
      ...
      const repositoryRequest: RepositoryRequest = this.createRepositoryRequest(
        createdProject.id,
      );
      ...
      try {
        ...
        throw new InternalServerErrorException(
          `Failed to create repository for project ${createdProject.id}`,
        );
      }
      ...
      const project = await this.prisma.project
        ...
        .catch(() => {
          throw new NotFoundException(
            `Project with uuid ${projectId} not found`,
          );
        });
      return project;
    }
  ...
  async delete(projectId: string, userId: string): Promise<SanitizedProject> {
    // Retrieve the project before deleting it
    const project = await this.prisma.project
      .findFirstOrThrow({ where: { id: projectId } })
      .catch(() => {
        throw new NotFoundException(`Project with uuid ${projectId} not found`);
      });
    ...
    return await this.prisma.$transaction(async (tx) => {
      ...
      if (!user.isAdmin && userId !== project.userId)
      ...
      const repositoryRequest: RepositoryRequest =
        this.createRepositoryRequest(projectId);
      try {
        await this.gitRepoManagerService.delete(repositoryRequest);
      } catch (e) {
        throw new InternalServerErrorException(
          `Failed to delete repository for project ${projectId}: ${e}`,
        );
      }
